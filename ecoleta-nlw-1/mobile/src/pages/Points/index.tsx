...
const Points = () => {
    ...
    //Pegar os items da API para fazer a scrollbar
    useEffect(() => {
      api.get('items').then(response =>{
        setItems(response.data);
      });
    }, []);
    //Pegar geolocalizacao do usuario para mostrar no mapa como ponto inicial
    useEffect(() => {
      async function loadPosition(){
        const {status} = await Location.requestPermissionsAsync();
        if(status !== 'granted'){
          Alert.alert('Oooooops...', 'A localização é essencial para o funcionamento da aplicação!');
          return;
        }
    ...
    //Pegar da API os pontos de coleta da região
    useEffect(() => {
      api.get('Points', {
        params: {
